...
def checkEvent(response):
    ...
    if ((response == "buy phase" or response == "iatch poin") and roundStart == False):
        print("Buying!")
        playAudio(os.path.abspath("test-python\\data\\voices\\buy phase"))
        roundStart = True
        deathHappened = False
        return
    elif ((response == "lost" or response == "lust") and roundStart == True):
        print("You Lost!")
        playAudio(os.path.abspath("test-python\\data\\voices\\lost"))
        roundStart = False
        return
    elif ((response == "clitph" or response == "clutch") and roundStart == True):
        print("You Clutched!!")
        playAudio(os.path.abspath("test-python\\data\\voices\\won"))
        roundStart = False
        return
    else:
        ...
        if ((response == "won" or response == "wun" ) and roundStart == True):
            ...
            return
              ...
              print("Death!")
              playAudio(os.path.abspath("test-python\\data\\voices\\death"))
              deathHappened = True
              return
